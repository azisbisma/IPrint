                            ...
                            <h3>Kategori</h3>
                                <ul class="list">
                                    @foreach(Helper::getAllCategory() as $cat)
                                        <li>
                                            <a href="{{ route('product-cat', $cat->slug) }}">{{$cat->title}}</a><span></span>
                                        </li>
                                    @endforeach
                                </ul>
                        </div>
                                    ...
                                    <div class="product-sorting">
                                        <label for="sorting">Urutkan Berdasarkan :</label>
                                            <select class="form-control" id="sorting">
                                                <option value="default" {{ $sort == 'default' ? 'selected' : '' }}>Pengurutan Bawaan</option>
                                                <option value="priceAsc" {{ $sort == 'priceAsc' ? 'selected' : '' }}>Harga Rendah - Tinggi</option>
                                                <option value="priceDesc" {{ $sort == 'priceDesc' ? 'selected' : '' }}>Harga Tinggi - Rendah</option>
                                                <option value="conditionNew" {{ $sort == 'conditionNew' ? 'selected' : '' }}>Baru</option>
                                                <option value="conditionUsed" {{ $sort == 'conditionUsed' ? 'selected' : '' }}>Bekas</option>
                                                <option value="titleAsc" {{ $sort == 'titleAsc' ? 'selected' : '' }}>A - Z</option>
                                                <option value="titleDesc" {{ $sort == 'titleDesc' ? 'selected' : '' }}>Z - A</option>
                                            </select>
                                        <h3 class="total-show-product">Menampilkan : <span>{{ $products->firstItem() }} - {{ $products->lastItem() }} item</span></h3>
                                    </div>
                                </div>
