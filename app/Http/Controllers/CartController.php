...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request){
        ...
        request()->session()->flash('success','Ditambahkan di keranjang');
        return back();       
    }  
    ...
    public function singleAddToCart(Request $request){
        ...
        if (!$product || $product->stock < $request->quantity) {
            return back()->with('error', 'Out of stock or invalid product.');
        }
        ...
        if ($already_cart) {
            $new_quantity = $already_cart->quantity + $request->quantity;
        ...
        request()->session()->flash('success', 'PDitambahkan di keranjang.');
        return back();
    }
    ...
    public function checkout(Request $request){
        ...
        // }
        $user=Auth::user();
        return view('shop.pages.checkout', compact('user'));
    }
    ...
    public function couponStore(Request $request){
        ...
        // dd($coupons);
        if(!$coupons){
            request()->session()->flash('error','Voucher tidak ditemukan');
            return back();
        }
        if($coupons){
            ...
            session()->put('coupon',[
                'id'=>$coupons->id,
                'code'=>$coupons->code,
                'value'=>$coupons->discount($total_price)
            ]);
            request()->session()->flash('success','Kupon berhasil dipakai');
